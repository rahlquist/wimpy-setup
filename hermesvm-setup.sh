...
# ── Primary inference: wimpy llama-hugs ───────────────────────────────────────
...
if curl -fsSL --max-time 5 "http://${WIMPY_HOST}:${LLAMA_PORT}/v1/models" \
    ...
    log "llama-hugs at $LLAMA_URL is reachable ✔"
else
    warn "Cannot reach $LLAMA_URL right now"
    warn "This is expected if llama-hugs isn't started yet on wimpy."
    warn "After adding models and starting llama-hugs on wimpy, verify with:"
